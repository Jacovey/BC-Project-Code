...
// BC5 Pneumatic Shield Controller
...
/*
 ...
 Paddress : 22 24 26 28 30 32 34 36 38 40  50 42/45
 Vaddress : 23 25 27 29 31 33 35 37 39 41 N/A 44/43
 Psensor  : A0 A1 A2 A3 A4 A5 A6 A7 A8

 NOTES FOR BC5:
*/

#define numChan 12              // Includes all channels (blower and master are final two channels)
#define numValveChan 9         // # of links
#define blowerRelay 52          // pin for blower relay
#define linearMotor 9
#define PRESSURE_TOLERANCE 0.5   // helps slow relay bouncing (generally .1-.5 psi)
#define MAXLINKPRESSURE 6      // max pressure in each link (generally 5)
...
static unsigned int pValvePins[numValveChan] = {22, 24, 26, 28, 30, 32, 34, 36, 38, 40};// Pressure valve pins (generally even numbered pins from 22 up)
static unsigned int vValvePins[numValveChan] = {23, 25, 27, 29, 31, 33, 35, 37, 39, 41};//   Vacuum valve pins (generally odd  numbered pins from 23 up)
static unsigned int masterVPins[2]           = {43, 44};//   Master valve pins (arranged {atmo inlet, atmo outlet})
//*************************************************************************************************************************
...
void handleRXCommand(int howmany) {
  ...
  for (int i = 0; i < howmany; i++) { // scan through each received command
    if (i < numChan){ // dont read more data than the number of channels?
      ...
      //Rack Control
      if (channel == numChan-3){
        if  (state == 102) {analogWrite(linearMotor, 0);}//   if (debug) Serial.println("RACK IN");}
        if  (state == 101) {analogWrite(linearMotor, 255);}// if (debug) Serial.println("RACK OUT");}
      }
      // Blower Control
      else if (i == numChan-2){// Blower is always second to last channel
